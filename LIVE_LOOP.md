...
#### UFW firewall hardening (run once on the VM)
...
#### SSH key for GitHub (run once on the VM)

```bash
ssh-keygen -t ed25519 -f ~/.ssh/id_ootils -C "ootils-live-vm"
cat ~/.ssh/id_ootils.pub  # Add this key to GitHub → Settings → SSH keys
```
...
> **Note :** Les migrations s'appliquent **automatiquement** au démarrage de l'API (via `OotilsDB.__init__`).
> Il n'y a **rien à lancer manuellement**. Pas de `docker compose exec api python -m ootils_core.db.migrate`.

Vérifie simplement qu'elles sont passées :
